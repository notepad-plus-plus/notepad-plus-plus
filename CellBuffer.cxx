	...
	if (currentAction >= 1) {
		if (0 == undoSequenceDepth) {
			...
			if ((currentAction == savePoint) || (currentAction == tentativePoint)) {
				...
				currentAction++;
			} else if (at == ActionType::remove || at == ActionType::clear) {
...
const char* CellBuffer::DeleteCharsClear(Sci::Position position, Sci::Position deleteLength, bool& startSequence) {
	// InsertString and DeleteChars are the bottleneck though which all changes occur
	PLATFORM_ASSERT(deleteLength > 0);
	const char* data = nullptr;
	if (!readOnly) {
		if (collectingUndo) {
			...
			// The gap would be moved to position anyway for the deletion so this doesn't cost extra
			data = substance.RangePointer(position, deleteLength);
			data = uh.AppendAction(ActionType::clear, position, data, deleteLength, startSequence);
		}
...
void CellBuffer::PerformUndoStep() {
	...
	} else if (actionStep.at == ActionType::remove || actionStep.at == ActionType::clear) {
