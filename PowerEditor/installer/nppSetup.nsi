...
${MementoSection} "Context Menu Entry" explorerContextMenu
	...
	!ifdef ARCH64
		...
		File /oname=$INSTDIR\NppShell.dll "..\bin64\NppShell.x64.dll"
	!else ifdef ARCHARM64
		File /oname=$INSTDIR\NppShell.msix "..\binarm64\NppShell.msix"
		File /oname=$INSTDIR\NppShell.dll "..\binarm64\NppShell.arm64.dll"
	!else
		; We need to test which arch we are running on, since 32bit exe can be run on both 32bit and 64bit Windows.
		${If} ${RunningX64}
    		File /oname=$INSTDIR\NppShell.dll "..\bin64\NppShell.x64.dll"
		${Else}
    		File /oname=$INSTDIR\NppShell.dll "..\bin\NppShell.x86.dll"
		${EndIf}    
