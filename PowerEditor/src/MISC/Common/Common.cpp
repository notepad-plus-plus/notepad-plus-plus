...
bool loadRawDataFromResource(const int iResourceIn, LPBYTE pBufferOut, DWORD& dwBufferSizeInOut, const HMODULE hModuleIn)
{
	HMODULE hModule = hModuleIn ? hModuleIn : ::GetModuleHandle(nullptr);

	HRSRC hResourceData = ::FindResource(hModule, MAKEINTRESOURCE(iResourceIn), RT_RCDATA);
	if (!hResourceData)
		return false;

	DWORD dwResourceSize = ::SizeofResource(hModule, hResourceData);
	if (dwResourceSize == 0)
		return false;

	if (!pBufferOut)
	{
		if (dwBufferSizeInOut == 0)
		{
			// ok, called only to find the required buffer size
			dwBufferSizeInOut = dwResourceSize;
			return true;
		}
		return false;
	}
	...
	HGLOBAL hglData = ::LoadResource(hModule, hResourceData); // not a real HGLOBAL, do not use with the GlobalLock
	if (hglData)
	{
		LPVOID pData = ::LockResource(hglData);
		if (pData)
		{
			::memcpy_s(pBufferOut, dwBufferSizeInOut, pData, dwResourceSize);
			bRet = true;
