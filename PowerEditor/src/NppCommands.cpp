...
void Notepad_plus::command(int id)
{
	switch (id)
	{
		...
		case IDM_EDIT_SORTLINES_LEXICO_CULTURE_SENS_ASCENDING:
		case IDM_EDIT_SORTLINES_LEXICO_CULTURE_SENS_DESCENDING:
		...
		case IDM_EDIT_SORTLINES_RANDOMLY:
		{
			...
			bool isDescending = id == IDM_EDIT_SORTLINES_LEXICOGRAPHIC_DESCENDING ||
								...
								id == IDM_EDIT_SORTLINES_LEXICO_CULTURE_SENS_DESCENDING;
			...
			else if (id == IDM_EDIT_SORTLINES_LEXICO_CULTURE_SENS_DESCENDING || id == IDM_EDIT_SORTLINES_LEXICO_CULTURE_SENS_ASCENDING)
			{
				pSorter = std::unique_ptr<ISorter>(new LexicographicIgnoreCaseCultureSensitiveSorter(isDescending, fromColumn, toColumn));
			}
