...
BufferID Notepad_plus::doOpen(const generic_string& fileName, bool isRecursive, bool isReadOnly, int encoding, const TCHAR *backupFileName, FILETIME fileNameTimestamp)
{
    ...
    else
    {
        if (globbing || ::PathIsDirectory(targetFileName.c_str()))
        {
            ...
            vector<generic_string> filePatterns;
            vector<generic_string> excludeDirPatterns;
            if (globbing)
            {
				...
				if (substring)
				{
					...
					filePatterns.push_back(substring + 1);
					generic_string dir(targetFileName.c_str(), pos + 1); // use char * to evoke:
																   // string (const char* s, size_t n);
																   // and avoid to call (if pass string) :
																   // string (const string& str, size_t pos, size_t len = npos);

					getMatchedFileNames(dir.c_str(), 0, excludeDirPatterns, filePatterns, fileNames, isRecursive, false);
				}
            }
            else
            {
				...
				filePatterns.push_back(TEXT("*"));

                getMatchedFileNames(fileNameStr.c_str(), 0, excludeDirPatterns, filePatterns, fileNames, true, false);
            }
