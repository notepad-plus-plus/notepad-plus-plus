...
void FindReplaceDlg::findAllIn(InWhat op)
{
	...
	if (::SendMessage(_hParent, cmdid, 0, 0))
	{
		...
		if (data.pszName)
		{
			data.pszAddInfo = const_cast<TCHAR *>(new TCHAR[nameLenMax]);
			lstrcpyn(const_cast<TCHAR *>(data.pszAddInfo), _findAllResultStr, nameLenMax);
		}
		if (_findAllResult) 
		{
			focusOnFinder();
...
Finder * FindReplaceDlg::createFinder(generic_string str2Search)
{
    ...
    {
        data.pszName = const_cast<TCHAR *>(new TCHAR[str2Search.length()]);
