...
void ScintillaEditView::init(HINSTANCE hInst, HWND hPere)
{
   ...
   _hSelf = ::CreateWindowEx(
	...
	if (::IsWindowUnicode(_hSelf))
	{
		...
		_scintillaDefaultProc = reinterpret_cast<WNDPROC>(::SetWindowLongPtrW(_hSelf, GWL_WNDPROC, reinterpret_cast<LONG_PTR>(scintillaStatic_Proc)));
	}
	else 
	{
		::SetWindowLongPtrA(_hSelf, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
		_callWindowProc = CallWindowProcA;
		_scintillaDefaultProc = reinterpret_cast<WNDPROC>(::SetWindowLongPtrA(_hSelf, GWL_WNDPROC, reinterpret_cast<LONG_PTR>(scintillaStatic_Proc)));
	}
