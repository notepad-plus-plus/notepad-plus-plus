...
namespace Utf8   // could be a static class, instead of a namespace, if it needs private members
{
    ...
    inline static bool isSingleByte(UCHAR c)
    {
        return c < 0x80;
    }
    inline static bool isPartOfMultibyte(UCHAR c)
    {
        return c >= 0x80;
    }
    inline static bool isFirstOfMultibyte(UCHAR c)
    {
        return c >= 0xC2 && c < 0xF5;    // 0xF5 to 0xFD are defined by UTF-8, but are not currently valid Unicode
    }
    inline static bool isContinuation(UCHAR c)
    {
        return (c & 0xC0) == 0x80;
    }
    inline static bool isValid(UCHAR c)
    {
        return c < 0xC0 || isFirstOfMultibyte(c);    // validates a byte, out of context
    }
