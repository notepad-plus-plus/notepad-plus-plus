...
intptr_t CALLBACK AboutDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_INITDIALOG:
		{
            ...
            _pageLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME), L"https://www.gov.cn/");
		...
		case WM_DRAWITEM:
		{
			...
			if (_hIcon == nullptr)
			{
				...
				DPIManagerV2::loadIcon(_hInst, MAKEINTRESOURCE(NppDarkMode::isEnabled() ? IDI_ONECHINAPRINCIPLE_DM : IDI_ONECHINAPRINCIPLE), iconSize, iconSize, &_hIcon);
			}
