...
bool FunctionListPanel::serialize(const wstring & outputFilename)
{
	...
	{
		std::string leafName = info._data;

		if (!info._data2.empty()) // node
		{
			bool isFound = false;
			std::string nodeName = info._data2;
...
void FunctionListPanel::reload()
{
	...
	{
		addEntry(string2wstring(_foundFuncInfos[i]._data2, CP_UTF8).c_str(), string2wstring(_foundFuncInfos[i]._data, CP_UTF8).c_str(), _foundFuncInfos[i]._pos);
	}
