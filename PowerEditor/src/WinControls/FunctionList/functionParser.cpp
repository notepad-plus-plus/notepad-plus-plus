...
{
	NppXml::Element elem = NppXml::toElement(classRangeParser);

	const char* mainExpr = NppXml::attribute(elem, "mainExpr");
	if (!mainExpr || !mainExpr[0])
		return false;
	mainExprStr = mainExpr;

	const char* openSymbol = NppXml::attribute(elem, "openSymbole");
	if (openSymbol && openSymbol[0])
		openSymboleStr = openSymbol;

	const char* closeSymbol = NppXml::attribute(elem, "closeSymbole");
	if (closeSymbol && closeSymbol[0])
		closeSymboleStr = closeSymbol;

	NppXml::Node classNameParser = NppXml::firstChildElement(classRangeParser, "className");
	if (classNameParser)
	{
	...
	NppXml::Node functionParser = NppXml::firstChildElement(classRangeParser, "function");
	if (!functionParser)
		return false;
	...
	NppXml::Node functionNameParser = NppXml::firstChildElement(functionParser, "functionName");
...
)
{
	...
	NppXml::Node classNameParser = NppXml::firstChildElement(functionParser, "className");
	if (classNameParser)
	{
...
bool FunctionParsersManager::loadFuncListFromXmlTree(const std::wstring& xmlDirPath, LangType lType, const std::wstring& overrideId, int udlIndex)
{
	...
	NppXml::Document xmlFuncListDoc = new NppXml::NewDocument();;
	const bool loadOK = NppXml::loadFile(xmlFuncListDoc, funcListRulePath.c_str());

	if (!loadOK)
	{
		delete xmlFuncListDoc;
		return false;
	}

	NppXml::Node root = NppXml::firstChildElement(xmlFuncListDoc, "NotepadPlus");
	if (!root)
	{
		delete xmlFuncListDoc;
		return false;
	...
	NppXml::Node parserRoot = NppXml::firstChildElement(root, "parser");
	if (!parserRoot)
	{
		delete xmlFuncListDoc;
		return false;
	...
	NppXml::Node classRangeParser = NppXml::firstChildElement(parserRoot, "classRange");
	NppXml::Node functionParser = NppXml::firstChildElement(parserRoot, "function");
	...
	else if (classRangeParser)
	{
		std::string mainExpr, openSymbole, closeSymbole, functionExpr;
		getZonePaserParameters(classRangeParser, mainExpr, openSymbole, closeSymbole, classNameExprArray, functionExpr, functionNameExprArray);
		_parsers[index]->_parser = new FunctionZoneParser(id, displayName, commentExpr.c_str(), mainExpr, openSymbole, closeSymbole, classNameExprArray, functionExpr, functionNameExprArray);
	}
	else if (functionParser)
	{
		std::string mainExpr;
		getUnitPaserParameters(functionParser, mainExpr, functionNameExprArray, classNameExprArray);
		_parsers[index]->_parser = new FunctionUnitParser(id, displayName, commentExpr.c_str(), mainExpr, functionNameExprArray, classNameExprArray);
	}
...
bool FunctionParsersManager::getOverrideMapFromXmlTree(const std::wstring& xmlDirPath)
{
	...
	NppXml::Node associationMapRoot = NppXml::firstChildElement(root, "associationMap");
	if (associationMapRoot) 
	{
			...
			childNode = NppXml::nextSiblingElement(childNode, "association"))
		{
			...
			if (langID > 0)
			{
				_parsers[langID] = new ParserInfo(string2wstring(id, CP_UTF8));
			}
			else if (userDefinedLangName && userDefinedLangName[0])
			{
				...
				{
					_parsers[_currentUDIndex] = new ParserInfo(string2wstring(id, CP_UTF8), string2wstring(userDefinedLangName, CP_UTF8));
				}
			}
		}
	}
...
void FunctionParser::funcParse(std::vector<foundInfo>& foundInfos, size_t begin, size_t end, ScintillaEditView **ppEditView, const std::string& classStructName, const std::vector<std::pair<size_t, size_t>>* commentZones)
{
	if (begin >= end)
		return;
...
std::string FunctionParser::parseSubLevel(size_t begin, size_t end, std::vector< std::string > dataToSearch, intptr_t & foundPos, ScintillaEditView **ppEditView)
{
	...
	(*ppEditView)->execute(SCI_SETSEARCHFLAGS, flags);
	const intptr_t targetStart = (*ppEditView)->searchInTarget(dataToSearch[0], begin, end);
...
void FunctionZoneParser::classParse(std::vector<foundInfo>& foundInfos, std::vector<std::pair<size_t, size_t>>& scannedZones, const std::vector<std::pair<size_t, size_t>>& commentZones, size_t begin, size_t end, ScintillaEditView** ppEditView, std::string /*classStructName*/)
{
	if (begin >= end)
		return;
...
void FunctionZoneParser::parse(std::vector<foundInfo>& foundInfos, size_t begin, size_t end, ScintillaEditView** ppEditView, std::string classStructName)
{
...
void FunctionUnitParser::parse(std::vector<foundInfo>& foundInfos, size_t begin, size_t end, ScintillaEditView** ppEditView, std::string classStructName)
{
...
void FunctionMixParser::parse(std::vector<foundInfo>& foundInfos, size_t begin, size_t end, ScintillaEditView** ppEditView, std::string classStructName)
{
