	...
	case WM_COMMAND:
	{
		...
		case IDM_BABYGRID_MODIFY:
		{
			...
			case STATE_MENU:
			{
				...
				if (csc.doDialog(_nativeLangSpeaker->isRTL()) != -1 && prevcsc != csc)
				{
					//shortcut was altered
					nppParam->addUserModifiedIndex(row - 1);
					shortcuts[row - 1] = csc;
			...
			case STATE_MACRO:
			{
				...
				if (msc.doDialog(_nativeLangSpeaker->isRTL()) != -1 && prevmsc != msc)
				{
					//shortcut was altered
					shortcuts[row - 1] = msc;
			...
			case STATE_USER:
			{
				...
				if (ucmd.doDialog(_nativeLangSpeaker->isRTL()) != -1 && prevucmd != ucmd)
				{
					//shortcut was altered
					shortcuts[row - 1] = ucmd;
			...
			case STATE_PLUGIN:
			{
				...
				if (pcsc.doDialog(_nativeLangSpeaker->isRTL()) != -1 && prevpcsc != pcsc)
				{
					//shortcut was altered
					nppParam->addPluginModifiedIndex(row - 1);
					shortcuts[row - 1] = pcsc;
			...
			case STATE_SCINTILLA:
			{
				...
				if (skm.doDialog(_nativeLangSpeaker->isRTL()) != -1 && prevskm != skm)
				{
					//shortcut was altered
		...
		case IDM_BABYGRID_DELETE:
		{
			...
			NativeLangSpeaker *_nativeLangSpeaker = (NppParameters::getInstance())->getNativeLangSpeaker();
			int result = _nativeLangSpeaker->messageBox("ShortcutMapperDelete", _hSelf,
				TEXT("Are you sure you want to delete this shortcut?"), TEXT("Are you sure?"),
