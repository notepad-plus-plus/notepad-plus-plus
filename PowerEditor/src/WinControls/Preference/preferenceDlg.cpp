...
intptr_t CALLBACK MiscSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch (message) 
	{
		case WM_INITDIALOG :
		{
			...
			::SendDlgItemMessage(_hSelf, IDC_CHECK_SUPPRESS2GBWARNING, BM_SETCHECK, nppGUI._suppress2GBWarning, 0);
		...
		case WM_COMMAND : 
		{
			...
			switch (wParam)
			{
				...
				{
					nppGUI._suppress2GBWarning = isCheckedOrNot(IDC_CHECK_SUPPRESS2GBWARNING);
					return TRUE;
				}
