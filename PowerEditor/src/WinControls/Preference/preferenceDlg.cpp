...
INT_PTR CALLBACK SearchingSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			::SendDlgItemMessage(_hSelf, IDC_CHECK_STYLEONLY1, BM_SETCHECK, nppGUI._styleOnlySingleOccurrence, 0);
		}
		break;
		...
		case WM_COMMAND:
		{
			switch (wParam)
			{
				...
				case IDC_CHECK_STYLEONLY1:
				{
					nppGUI._styleOnlySingleOccurrence = isCheckedOrNot(IDC_CHECK_STYLEONLY1);
					return TRUE;
				}
				break;
