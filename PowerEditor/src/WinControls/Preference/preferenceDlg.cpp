...
void PreferenceDlg::changeAllToolTipsText()
{
	...
	changeToolTipText(IDC_BUTTON_NPC_LAUNCHSTYLECONF, hEditSubDlg, _editingSubDlg._tipNpcColor, pNativeSpeaker->getLocalizedStrFromID(EDITING_TIPNPCCOLOR_NODENAME, EDITING_TIPNPCCOLOR_TEXT).c_str());
...
intptr_t CALLBACK EditingSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_INITDIALOG :
		{
			...
			generic_string tipNpcCol2show = pNativeSpeaker->getLocalizedStrFromID(EDITING_TIPNPCCOLOR_NODENAME, EDITING_TIPNPCCOLOR_TEXT);
