...
void EditingSubDlg::initScintParam()
{
	...
	::SendDlgItemMessage(_hSelf, IDC_CHECK_ENABLEDRAGDROP, BM_SETCHECK, svp._enableDragDrop, 0);
}
	...
	switch (message)
	{
		...
		case WM_COMMAND : 
		{
			switch (wParam)
			{
				...
				case IDC_CHECK_ENABLEDRAGDROP:
				{
					bool isChecked = isCheckedOrNot(IDC_CHECK_ENABLEDRAGDROP);
					svp._enableDragDrop = isChecked;
					::SendMessage(::GetParent(_hParent), NPPM_INTERNAL_ENABLEDRAGDROP, 0, 0);
					return TRUE;
				}
