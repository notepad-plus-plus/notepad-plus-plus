...
const int CARETLINEFRAME_SMALLEST = 0;
...
void EditingSubDlg::initScintParam()
{
	...
	::EnableWindow(::GetDlgItem(_hSelf, IDC_CARETLINEFRAME_WIDTH_SLIDER), svp._currentLineHilitingShow);
	showFrameWidthText(svp._currentLineFrameWidth);
...
void EditingSubDlg::showFrameWidthText(int width)
{
	std::wstring frameWidthText{};

	frameWidthText = std::to_wstring(width) + L" px " + ((width == 0) ? L"(No Frame)" : L"wide");

	::SetDlgItemText(_hSelf, IDC_CARETLINEFRAME_WIDTH_DISPLAY, frameWidthText.c_str());
}
...
intptr_t CALLBACK EditingSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		...
		case WM_CTLCOLORSTATIC:
		{
			...
			{
				return NppDarkMode::onCtlColorDarkerBGStaticText(reinterpret_cast<HDC>(wParam),
					isCheckedOrNot(IDC_CHECK_CURRENTLINEHILITE));
			}
