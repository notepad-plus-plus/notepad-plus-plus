...
std::wstring resolveExecutablePath(const std::wstring& cmd)
{
	if (isAbsolutePath(cmd))
	{
		return doesPathExist(cmd.c_str()) ? cmd : L"";
	}
	...
	DWORD result = SearchPathW(
		nullptr,        // search %PATH%
		cmd.c_str(),    // bare filename
		L".exe",        // default extension if missing
		MAX_PATH,
		resolved,
		&filePart
	);

	return (result > 0) ? std::wstring(resolved) : L"";
}

bool isInTrustedDirectory(const std::wstring& absolutePath)
{
	const DWORD trustedCSIDLs[] = {
	...
	for (DWORD csidl : trustedCSIDLs)
	{
		if (SUCCEEDED(::SHGetFolderPathW(nullptr, csidl, nullptr, SHGFP_TYPE_CURRENT, rootPath)))
		{
			// Case-insensitive prefix check
			if (::PathIsPrefix(rootPath, absolutePath.c_str()))
				return true;
		}
	}
...
bool isRiskyExecutableByExt(const std::wstring& path)
{
	static const std::vector<std::wstring> riskyExtensions = {
		L".exe", L".bat", L".cmd", L".js", L".vbs", L".msi", L".ps1",
		L".com", L".scr", L".hta", L".lnk", L".wsf", L".cpl", L".url"
	};
	...
	for (const auto& risky : riskyExtensions)
	{
		if (extLower == risky)
			return true;
	}
...
bool isURL(const std::wstring& cmd)
{
	return (cmd.rfind(L"https://", 0) == 0 || cmd.rfind(L"http://", 0) == 0) ;
}

bool isSafeURL(const std::wstring& cmd)
{
	return cmd.rfind(L"https://", 0) == 0;
}
...
HINSTANCE Command::run(HWND hWnd, const wchar_t* cwd)
{
	...
	if (isURL(resolvedCmd))
	{
		if (!isSafeURL(resolvedCmd))
		{
			int choice = pNativeLangSpeaker->messageBox("RunSecurityNotSafeURL",
				hWnd,
				L"This command is a URL with an unsecure protocol (http):\r\"$STR_REPLACE$\"\rThis may indicate a misconfiguration or tampering.\r\rProceed anyway?",
				L"Run - Security Warning",
	...
	{
		if (!isAbsolutePath(resolvedCmd))
		{
			// Resolve bare names (e.g. "calc.exe") to full path via %PATH%
			wchar_t resolved[MAX_PATH]{};
			wchar_t* filePart = nullptr;
			DWORD found = ::SearchPathW(nullptr, resolvedCmd.c_str(), L".exe", MAX_PATH, resolved, &filePart);
			if (found == 0 || found >= MAX_PATH)
			{
				pNativeLangSpeaker->messageBox("RunSecurityCmdNotFound",
					hWnd,
					L"Command not found or path too long:\r\"$STR_REPLACE$\"",
					L"Run - Security Error",
		...
		// Require absolute path after resolution
		if (!isAbsolutePath(resolvedCmd))
		{
			pNativeLangSpeaker->messageBox("RunSecurityOnlyAbsolutePathAllowed",
				hWnd,
				L"Only absolute paths are allowed:\r\"$STR_REPLACE$\" is invalid command.",
				L"Run - Security Error",
				MB_ICONWARNING | MB_OK | MB_APPLMODAL,
				0,
				resolvedCmd.c_str());
		...
		if (!doesDirectoryExist(resolvedCmd.c_str()))	// if it's a directory, we allow it to be run without warning, even if it's outside trusted directories,
														// since there is no executable to be tampered with; we just open the folder in Explorer
		{
			if (!doesFileExist(resolvedCmd.c_str()))
			{
				pNativeLangSpeaker->messageBox("RunSecurityFileNoExist",
					hWnd,
					L"File does not exist:\r\"$STR_REPLACE$\"",
					L"Run - Security Error",
					MB_ICONWARNING | MB_OK | MB_APPLMODAL,
					0,
					resolvedCmd.c_str());
			...
			else // if it's a file, we check if it has a risky extension, and warn if it's outside trusted directories
			{
				// Check if the file is executable (e.g. .exe, .bat, .cmd, .js, etc.)
				if (isRiskyExecutableByExt(resolvedCmd))
				{
					// Warn if outside trusted directories (Program Files, Windows, etc.)
					if (!isInTrustedDirectory(resolvedCmd))
					{
						int choice = pNativeLangSpeaker->messageBox("RunSecurityNoTrustedLocation",
							hWnd,
							L"This command will run an executable file from an untrusted location:\r\"$STR_REPLACE$\"\rThis may indicate a misconfiguration or tampering.\r\rProceed anyway?",
							L"Run - Security Warning",
							MB_ICONWARNING | MB_YESNO | MB_APPLMODAL,
							0,
							resolvedCmd.c_str());
	...
	// Write back resolved absolute path so ShellExecute gets the canonical form
