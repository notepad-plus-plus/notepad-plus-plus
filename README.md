...
Here are the instructions to build SciLexer.dll (for both 32-bit & 64-bit) for Notepad++:
 ...
 1. Download the [Boost source code](https://sourceforge.net/projects/boost/files/boost/1.55.0/).
    v1.55 should be used with VS 2015. Then unzip it. In my case, `boost_1_55_0` is copied in `C:\sources\`
 2. Go to `scintilla\boostregex\` then run BuildBoost.bat with your boost path.
	...
	If you are compiling a 64 bit Scintilla under your *VS2015 x64 Native tool command prompt*, add `-x64` flag.
...
To build SciLexer.dll without PCRE support (for both 32-bit & 64-bit):
 ...
 1. For 32-bit, open a command prompt *for building* ([a.k.a. the *Developer Command Prompt for VS2015*](https://msdn.microsoft.com/en-us/library/f2ccy3wt.aspx))
    ...
    - From the Windows Start screen/menu, type `Developer Command Prompt for VS2015`,
	...
	For 64-bit, open *VS2015 x64 Native tool command prompt*.
