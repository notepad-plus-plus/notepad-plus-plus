...
LRESULT CALLBACK FontSizeEditProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_KEYDOWN && wParam == VK_RETURN)
	{
		...
		if (hDialog)
		{
			WordStyleDlg* pDlg = reinterpret_cast<WordStyleDlg*>(::GetWindowLongPtr(hDialog, GWLP_USERDATA));
			if (pDlg)
			{
				pDlg->onFontSizeEnter();
			}
		}
		return 0;
	}
...
void WordStyleDlg::onFontSizeEnter()
{
	updateFontSize();
	notifyDataModified();
...
intptr_t CALLBACK WordStyleDlg::run_dlgProc(UINT Message, WPARAM wParam, LPARAM lParam)
{
	switch (Message)
	{
		case WM_INITDIALOG :
		{
			...
			HWND hEdit = NULL;
			HWND hChild = GetWindow(_hFontSizeCombo, GW_CHILD);
			while (hChild)
			{
				wchar_t className[256];
				GetClassName(hChild, className, sizeof(className) / sizeof(className[0]));
			...
			if (hEdit)
			{
				originalProc = (WNDPROC)SetWindowLongPtr(hEdit, GWLP_WNDPROC, (LONG_PTR)&FontSizeEditProc);
			}
...
void WordStyleDlg::updateFontSize()
{
	...
	constexpr size_t intStrLen = 8;
	wchar_t intStr[intStrLen]{};

	// Get the text inside the combo box (typed or selected)
	::GetWindowText(_hFontSizeCombo, intStr, intStrLen);
	...
	int fontSize = static_cast<int>(wcstol(intStr, &endPtr, 10));

	// Validate conversion: check that entire string was numeric and font size is positive
	if (*endPtr == L'\0' && fontSize > 0)
	{
...
{
	updateFontSize(); // force fetch typed input
