	...
	if (enteredModification == 0) {
		...
		if (!cb.IsReadOnly()) {
			...
			for (int step = 0; step < steps; step++) {
				...
				if (action.at == ActionType::remove || action.at == ActionType::clear) {
...
bool Document::DeleteCharsClear(Sci::Position pos, Sci::Position len) {
	if (pos < 0)
		return false;
	if (len <= 0)
		return false;
	...
	if (enteredModification != 0) {
		...
		if (!cb.IsReadOnly()) {
			...
			const bool startSavePoint = cb.IsSavePoint();
			bool startSequence = false;
			const char* text = cb.DeleteCharsClear(pos, len, startSequence);
			if (startSavePoint && cb.IsCollectingUndo())
				NotifySavePoint(false);
			if ((pos < LengthNoExcept()) || (pos == 0))
				ModifiedAt(pos);
		...
		if (!cb.IsReadOnly()) {
			...
			for (int step = 0; step < steps; step++) {
				...
				if (action.at == ActionType::insert) {
					newPos += action.lenData;
					modFlags |= ModificationFlags::InsertText;
				} else if (action.at == ActionType::remove || action.at == ActionType::clear) {
					modFlags |= ModificationFlags::DeleteText;
				}
				if (steps > 1)
					modFlags |= ModificationFlags::MultiStepUndoRedo;
...
void Document::DelCharClear(Sci::Position pos) {
	DeleteCharsClear(pos, LenChar(pos));
}
