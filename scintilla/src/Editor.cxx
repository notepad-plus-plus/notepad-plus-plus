...
void Editor::Clear() {
	...
	if (sel.Empty()) {
		...
		UndoGroup ug(pdoc, (sel.Count() > 1) || singleVirtual); //Remove second parameter to prevent grouping of clears (del key)
		for (size_t r=0; r<sel.Count(); r++) {
			if (!RangeContainsProtected(sel.Range(r).caret.Position(), sel.Range(r).caret.Position() + 1)) {
				...
				if ((sel.Count() == 1) || !pdoc->IsPositionInLineEnd(sel.Range(r).caret.Position())) {
					pdoc->DelCharClear(sel.Range(r).caret.Position());
					sel.Range(r).ClearVirtualSpace();
...
void Editor::DelCharBack(bool allowLineStartDeletion) {	RefreshStyleData();
	if (!sel.IsRectangular())
		FilterSelections();
	if (sel.IsRectangular())
