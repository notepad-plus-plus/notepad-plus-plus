...
TEST_CASE("CellBuffer") {
	...
	SECTION("UndoRedoClear") {
		...
		REQUIRE(cb.CanUndo());
		REQUIRE(!cb.CanRedo());
		const char* cpDeletion = cb.DeleteCharsClear(1, 2, startSequence);
		REQUIRE(startSequence);
		REQUIRE(memcmp(cpDeletion, sTextDeleted, strlen(sTextDeleted)) == 0);
		REQUIRE(memcmp(cb.BufferPointer(), sTextAfterDeletion, strlen(sTextAfterDeletion)) == 0);
		REQUIRE(cb.CanUndo());
...
TEST_CASE("CharacterIndex") {
	...
	SECTION("Clear") {
		...
		cb.DeleteCharsClear(5, 1, startSequence);
		...
		cb.DeleteCharsClear(1, 4, startSequence);
	...
	SECTION("Clear Multiple lines") {
		...
		// Delete first 2 new lines -> "az\nc"
		cb.DeleteCharsClear(1, strlen(data) - 4, startSequence);
	...
	SECTION("Clear Complex") {
		...
		// "a\n\x90\x8D\x88\nz"
		// line 1 becomes 4 single byte characters
		cb.DeleteCharsClear(2, 1, startSequence);
		...
		// "a\x90\x8D\x88\nz"
		// Only 2 lines with line 0 containing 5 single byte characters
		cb.DeleteCharsClear(1, 1, startSequence);
